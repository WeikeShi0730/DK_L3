...
int main(void)
{
  ...
  fprintf(fp, ("Call wait time(min) ,"));
  fprintf(fp, ("Call wait less than t sec %%,"));
  ...
  int WAITING_TIME_LIMIT_LIST[] = {WAITING_TIME_LIMIT};
  ...
  {
    for (int l = 0; l < (sizeof(NUMBER_OF_CHANNELS_LIST) / sizeof(int)); l++)
    {
      for (int n = 0; n < (sizeof(WAITING_TIME_LIMIT_LIST) / sizeof(int)); n++)
      {
        ...
        for_avg_acc.call_wait_time = 0.0;
        for_avg_acc.call_wait_less_than_t = 0;
        ...
        while (random_seed != 0)
        {
          ...
          data.call_wait_time = 0.0;
          data.call_wait_less_than_t = 0;
          data.t = WAITING_TIME_LIMIT_LIST[n];
          data.random_seed = random_seed;

          /* Create the channels. */
          data.buffer = fifoqueue_new();
          data.channels = (Channel_Ptr *)xcalloc((int)NUMBER_OF_CHANNELS_LIST[l],
                                                 sizeof(Channel_Ptr));

          /* Initialize the channels. */
          for (int i = 0; i < NUMBER_OF_CHANNELS_LIST[l]; i++)
          {
          ...
          for_avg_acc.call_wait_time += data.call_wait_time;
          for_avg_acc.call_wait_less_than_t += data.call_wait_less_than_t;
          /* Clean up memory. */
        ...
        for_avg_acc.call_wait_time /= size_rand_seed;
        for_avg_acc.call_wait_less_than_t /= size_rand_seed;
        ...
        //fprintf(fp, ("Call wait time(min) ,"));
        fprintf(fp, "%f, ", for_avg_acc.call_wait_time / for_avg_acc.number_of_calls_processed);

        //fprintf(fp, ("Call wait less than t sec %%,"));
        fprintf(fp, "%f, ", (double)for_avg_acc.call_wait_less_than_t * 100 / for_avg_acc.number_of_calls_processed);
        ...
        printf("Call wait time(min) = %f \n", for_avg_acc.call_wait_time / for_avg_acc.number_of_calls_processed);
        printf("Call waited less than %d seconds = %f%% \n", WAITING_TIME_LIMIT_LIST[n], (double)for_avg_acc.call_wait_less_than_t * 100 / for_avg_acc.number_of_calls_processed);
